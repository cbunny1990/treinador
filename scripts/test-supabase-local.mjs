...
if (status.error || status.status !== 0) {
  process.stderr.write("Supabase local não está disponível. Inicia a stack com `npx supabase start` e volta a executar este teste.\n");
...
const result = spawnSync(process.execPath, ["--test", "tests/remote_workspace_supabase.local.test.js"], {
  cwd: root,
  stdio: "inherit",
  windowsHide: true,
  env: {
