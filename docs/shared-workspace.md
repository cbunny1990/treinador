...
Auditoria read-only adicional em 23/09/2026: `workspace_records`, `media_assets` e `teams` constam da publicação `supabase_realtime`, mas `activity_log` não constava, embora o cliente sincronize essa tabela. O cliente também subscreve `activity_log`, e a migração local `20260923120000_workspace_activity_realtime.sql` adiciona a tabela à publicação. Na validação anterior, a stack local aplicou as migrações e o lint SQL passou. Depois de alinhar os ficheiros locais com o histórico remoto, uma segunda stack isolada registou o início da aplicação das 19 migrações, mas não chegou a ficar saudável: o Docker deixou de responder a `docker ps` e o Supabase local padrão também foi reportado como `unhealthy`. Por isso, a cadeia alinhada ainda precisa de validação completa e o teste local de integração precisa de ser repetido. Nenhuma migração foi aplicada ao projeto publicado. O projeto confirmou RLS nas tabelas partilhadas e bucket `team-media` privado; o advisor também sinalizou que a proteção de palavras-passe comprometidas está desativada.
...
Reconciliação pré-deploy, read-only (23/09/2026): foram recuperados os 17 ficheiros com as versões registadas pelo Supabase MCP em produção, incluindo as sete migrações base e `20260922103529_enable_workspace_realtime`. A pasta local contém agora essas 17 versões mais as duas migrações pendentes `20260923120000` e `20260923120100`; os antigos baselines consolidados e migrações de continuidade com versões posteriores foram removidos da árvore de trabalho para evitar divergência de histórico. Os 19 ficheiros locais coincidem com a exportação usada na auditoria, incluindo quatro migrações históricas alinhadas após revisão. A cadeia foi processada numa stack isolada, mas o Docker ficou sem responder antes de ser possível confirmar saúde, lint e listagem local. A tentativa anterior de `supabase migration list --linked` falhou porque este worktree não está ligado ao projeto. Nenhuma ligação, reparação, aplicação remota ou `db push` foi feita. Antes de qualquer publicação, repetir a validação da cadeia completa e confirmar que apenas as duas novas migrações aparecem como pendentes.
