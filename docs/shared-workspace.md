...
Os blocos de um treino usam o UUID remoto do exercício ao sair deste dispositivo. Um plano legado que ainda contenha o ID numérico local é convertido pela ligação ao exercício da mesma equipa; se o exercício já não existir ou pertencer a outra equipa, o treino fica em conflito e não é enviado com uma referência inválida. Os exercícios são sincronizados antes dos treinos, para que um plano novo possa referir o exercício já persistido. A base local original não é alterada por esta conversão de payload.
