...
test('read returns unknown score as null and exposes no inferred counts or photos',async()=>{const f=fixture(),out=await f.call('get_match_analysis');assert.equal(out.result,null);assert.equal(out.statistics.event_count,0);assert.equal(f.calls.length,0);});
test('proposal requires explicit confirmation/current revision and remains separate from coach text',async()=>{const f=fixture();const proposal={summary:'Possível melhoria',hypotheses:['Hipótese'],next_priority:'Apoio',evidence_ids:[]};await assert.rejects(f.call('prepare_match_analysis',{confirmed:false,proposal}),/schema|confirmation/i);await f.call('prepare_match_analysis',{confirmed:true,proposal});assert.equal(f.match.payload.post_game.analysis.fields.observations,'Facto escrito pelo treinador');assert.equal(f.match.payload.post_game.analysis.agent_proposal.prepared_by,'Head Coach');assert.equal(f.calls.length,1);});
