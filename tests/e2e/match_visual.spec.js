...
test('visual match identifies unavailable callups and blocks start until all positions use eligible players',async({page})=>{
 await page.setViewportSize({width:390,height:844});const f=await seed(page);
 await page.evaluate(async refs=>{for(const [ref,status] of [[refs[1],'lesionado'],[refs[6],'ausente']]){const player=(await DB.listar('jogadores')).find(row=>row.sync_id===ref);await DB.modificar('jogadores',player.id,row=>({...row,estado_disponibilidade:status}));}await router();},f.refs);
 const unavailable=page.locator('[data-match-unavailable-callup]');await expect(unavailable).toContainText('Atleta 2 · Lesionado');await expect(unavailable).toContainText('Atleta 7 · Ausente');
 const startButton=page.getByRole('button',{name:'Iniciar jogo e contar minutos',exact:true});await expect(startButton).toBeDisabled();
 const lineup=page.locator('[data-match-form="lineup"]');expect(await lineup.locator('[name="def"] option').evaluateAll(options=>options.map(option=>option.value))).not.toContain(f.refs[1]);await lineup.locator('[name="def"]').selectOption(f.refs[5]);await lineup.getByRole('button',{name:'Guardar alinhamento visual',exact:true}).click();await saved(page);await expect(startButton).toBeEnabled();
 expect(await page.evaluate(id=>DB.obter('jogos',id).then(row=>row.callup.player_ids.length),f.id)).toBe(7);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();
});
