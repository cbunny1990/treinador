...
function pendingKnowledgeJobs(){
 return Array.from({length:32},(_,i)=>{const source=match();source.id=`90000000-0000-4000-8000-${String(i+1).padStart(12,'0')}`;return{source_id:source.id,source_updated_at:source.updated_at,source_kind:source.kind,payload:source.payload,claim_token:`91000000-0000-4000-8000-${String(i+1).padStart(12,'0')}`};});
}
function withIndexingBacklog(admin){
 const rpc=admin.rpc.bind(admin);admin.rpc=async(name,args)=>{if(name==='claim_team_knowledge_jobs')return{data:pendingKnowledgeJobs()};if(name==='replace_team_knowledge_source')return{data:args.p_chunks.length};if(name==='release_team_knowledge_jobs')return{data:args.p_claims.length};return rpc(name,args);};return admin;
}
...
test('RAG validates exact team/scope/filters and searches only with the authorized team UUID',async()=>{
 ...
 assert.equal(output.retrieval_status,'ready');assert.equal(output.results.length,1);assert.equal(output.results[0].source.ref,SOURCE);assert.equal(output.results[0].source.evidence_type,'coach_observation');
  ...
  const pendingOutput=await rag.executeTeamKnowledgeTool(withIndexingBacklog(fakeAdmin()),connector,'search_team_knowledge',{query:'O que correu mal?',source_kinds:['match']},{provider});
  assert.equal(pendingOutput.indexing.pending,true);assert.equal(pendingOutput.indexing.status,'batch_limit_reached_may_have_more');assert.equal(pendingOutput.evidence_status,'possibly_partial_sources');assert.match(pendingOutput.guidance,/pode haver mais/);
  const pendingEmptyAdmin=withIndexingBacklog(fakeAdmin()),pendingRpc=pendingEmptyAdmin.rpc.bind(pendingEmptyAdmin);pendingEmptyAdmin.rpc=async(name,args)=>name==='search_team_knowledge_chunks'?{data:[]}:pendingRpc(name,args);
  const pendingWithoutEvidence=await rag.executeTeamKnowledgeTool(pendingEmptyAdmin,connector,'search_team_knowledge',{query:'Que problemas se repetem?'},{provider});
  assert.equal(pendingWithoutEvidence.evidence_status,'indexing_may_be_incomplete');assert.deepEqual(pendingWithoutEvidence.results,[]);assert.match(pendingWithoutEvidence.guidance,/não infiras ausência histórica/);
