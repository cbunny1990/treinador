...
(function(root){
  ...
  async function summary(training){
    ...
    const review=C.effectiveReview(training);
    html+='<section class="panel hero-main section"><h3>Continuidade do Head Coach</h3><p class="lead">'+(review.status==='done'?esc(progress.label):'Preenche a avaliação para preparar a próxima sessão sem perder o contexto.')+'</p>'+(progress.outcome?'<p>Resultado assinalado pelo treinador: <strong>'+esc(progress.outcome)+'</strong></p>':'')+'<a class="btn accent section" href="#/continuidade/'+training.id+'">Avaliação → próximo treino</a></section>';
    return html;
  }
