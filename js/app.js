...
async function viewWorkspace(options){
  ...
  var currentWeekPlans=reportSeasonDocs.filter(function(d){if(d.type!=="weekly_plan"||d.status==="archived")return false;try{return TeamDevelopment.week(d).week_start===currentWeekStart;}catch(_){return false;}});
  var currentWeekFocusHTML;
  if(currentWeekPlans.length>1)currentWeekFocusHTML='<div class="notice" data-workspace-current-week><strong>Plano semanal por rever</strong><p>Existem '+currentWeekPlans.length+' planos para esta semana. Abre Semana e evolução para comparar; o Workspace não escolhe um silenciosamente.</p><a class="link" href="#/evolucao">Abrir Semana e evolução</a></div>';
  else if(currentWeekPlans.length===1){var currentWeek=TeamDevelopment.week(currentWeekPlans[0]);currentWeekFocusHTML='<div class="list-item row" data-workspace-current-week><span class="grow"><span class="title">'+esc(currentWeek.objective||"Semana sem objetivo definido")+'</span><span class="meta">Semana de '+fmtDate(currentWeek.week_start)+' · '+(String(currentWeek.evaluation?.summary||"").trim()?"Avaliação registada":"Avaliação por preencher")+'</span></span><a class="link" href="#/evolucao">Abrir semana</a></div>';}
  else currentWeekFocusHTML='<div class="empty" data-workspace-current-week>Sem objetivo semanal registado para esta semana. <a class="link" href="#/evolucao">Preparar semana</a></div>';
