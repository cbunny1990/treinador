...
async function viewWorkspace(){
  ...
  var pendingProposals=pendingTrainingProposals.concat(pendingTeamProposals);
  ...
  attention+='<section class="panel"><h3>Propostas por rever · '+pendingProposals.length+'</h3>'+(pendingProposals.length?'<div class="list">'+pendingProposals.slice(0,4).map(function(item){return '<a class="list-item" href="'+(item.kind==="team"?'#/evolucao':'#/continuidade/'+item.id)+'">'+(item.date?fmtDate(item.date)+' · ':'')+esc(item.title)+'</a>';}).join('')+'</div>':'<p class="empty">Sem propostas pendentes.</p>')+'</section>';
...
app.addEventListener("click",async function(event){
  ...
  if(action==="edit-team-record"){
    ...
    else{form.elements.namedItem("title").value=value.title;form.elements.namedItem("identified_at").value=value.identified_at;form.elements.namedItem("stage").value=value.stage;form.querySelectorAll('[name="session_refs"]').forEach(function(input){var parts=input.value.split(":");input.checked=value.sessions.some(function(x){return x.type===parts[0]&&x.id===parts.slice(1).join(":");});});form.querySelectorAll('[name="exercise_refs"]').forEach(function(input){input.checked=value.exercises.some(function(x){return x.id===input.value;});});form.elements.namedItem("observations").value=value.observations;form.elements.namedItem("interpretation").value=value.interpretation;form.elements.namedItem("hypothesis").value=value.hypothesis;form.elements.namedItem("evaluation").value=value.evaluation||"";form.elements.namedItem("coach_decision").value=value.coach_decision;}
...
app.addEventListener("submit",async function(event){
  ...
  if(type==="team-goal"){
    try{var goalDoc=fd.get("doc_id")?await WorkspaceStore.getDocument(Number(fd.get("doc_id"))):null,sessionRefs=fd.getAll("session_refs").map(function(v){var i=v.indexOf(":");return{type:v.slice(0,i),id:v.slice(i+1)};}),exerciseRows=await DB.porIndice("exercicios","team_id",DEFAULT_TEAM_ID),exerciseRefs=fd.getAll("exercise_refs").filter(function(ref){return exerciseRows.some(function(x){return String(x.sync_id)===String(ref);});}).map(function(ref){return{type:"exercise",id:ref};}),goalData=TeamDevelopment.saveGoal(goalDoc,{title:fd.get("title"),identified_at:fd.get("identified_at"),stage:fd.get("stage"),sessions:sessionRefs,evidence:sessionRefs,exercises:exerciseRefs,observations:fd.get("observations"),interpretation:fd.get("interpretation"),hypothesis:fd.get("hypothesis"),evaluation:fd.get("evaluation"),coach_decision:fd.get("coach_decision")},{expected_revision:Number(fd.get("expected_revision"))});var goalRefs=sessionRefs.concat(exerciseRefs);await saveTeamDevelopmentDocument(form,"team_goal",goalData,goalData.title,goalData.identified_at,goalRefs);return router();}catch(error){var goalFeedback=form.querySelector("[data-team-feedback]");goalFeedback.textContent=error.message;goalFeedback.hidden=false;return;}
  }
  if(type==="season"){
