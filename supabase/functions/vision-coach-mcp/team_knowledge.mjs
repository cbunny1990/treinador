...
function sourceFields(row){
  ...
  if(kind==='match'){
    ...
    const date=isoDate(p.data),session=obj(p.session),review=obj(p.review||session.review);
...
async function embed(inputs,{apiKey=globalThis.Deno?.env?.get?.('OPENAI_API_KEY'),fetchImpl=globalThis.fetch}={}){
  ...
  for(let start=0;start<inputs.length;start+=64){
    ...
    const response=await fetchImpl('https://api.openai.com/v1/embeddings',{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model:MODEL,dimensions:DIMENSIONS,input:batch})});
...
async function getTrainingPlanningContext(admin,connector,args,{provider={}}={}){
  ...
  if(recentMatches.length){
     retrievals.push(await executeTeamKnowledgeTool(admin,connector,'search_team_knowledge',{query:question,source_kinds:['match'],match_refs:recentMatches.map(row=>row.id),per_match_limit:2,limit:12},{provider}));
  }
