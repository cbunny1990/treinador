...
async function getRecentMatchContext(admin,connector,args,{provider={}}={}){
  ...
  const evidenceStatus=evidence.length?'sources_found':retrieval.retrieval_status==='sensitive_query_not_sent'?'sensitive_query_not_sent':retrieval.retrieval_status==='provider_not_configured'?'provider_not_configured':hasRetrievalUnavailable?'retrieval_unavailable':'insufficient_information';
  return {schema:'vision-recent-match-context@1',team_id:teamId,selection:{requested:requestedCount,returned:matches.length,criterion:'completed_matches_with_valid_date_on_or_before_today',missing_dated_matches_excluded:true},matches:matchFacts,
    semantic_evidence:evidence,semantic_retrieval_status:retrieval.retrieval_status,evidence_status:evidenceStatus,missing_data:{completed_matches:matches.length===0,structured_event_counts:matches.length===0||matchFacts.some(x=>!x.events_available),semantic_evidence:evidence.length===0,semantic_retrieval:hasRetrievalUnavailable},
