...
const PLANNING_CONTEXT_TOOL={name:'get_training_planning_context',description:'Build read-only hybrid context for a planned training date: the exact scheduled training if present, age group, operational roster availability, up to five latest completed matches, up to three recent training sessions, exercise usage and short cited RAG evidence from those matches and recent coaching/training material. Use before giving training advice. It may refresh the derived index, but never creates or changes a sports record or plan; returns missing-data flags.',inputSchema:{type:'object',properties:{target_date:{type:'string',format:'date'},question:{type:'string',minLength:2,maxLength:1200}},required:['target_date','question'],additionalProperties:false},annotations:{readOnlyHint:false,destructiveHint:false}};
...
const HEALTH_TEXT=/(?:lesao|injur|fratur|fractur|tendin|entors|sprain|strain|ligament|concuss|contus|bruis|distens|contractur|ruptur|luxac|dislocat|inflamac|cirurg|operac|fisioterap|reabilitac|diagnost|tratament|medic|saude|doenca|sintoma|dor muscular|dor no\s|dor de\s|\bpain\b|alerg|allerg|asma|epilep|diabet|cardiac|respirator|atestado|baixa medica|hipertens|hypertens|hipotens|hypotens|pressao arterial|tensao arterial|blood pressure|arterial pressure|hipoglicem|hiperglicem|hypoglyc|hyperglyc|glicemia|glucose|blood sugar|saude mental|mental health|ansiedade|ansioso|ansiosa|anxiet|depress|tdah|adhd|bipolar|esquizofren|schizophren|autismo|autista|autism|ataque de panico|panic attack|fobia|phobia|febr|fever|vomit|nause|diarre|diarrh|cefale|headache|enxaquec|migraine|desidrat|dehydrat|covid|varicela|chickenpox)/i;
...
async function getTrainingPlanningContext(admin,connector,args,{provider={}}={}){
  ...
  const recentTrainings=priorTrainings.filter(row=>{const date=recordDate(row);return date&&date<targetDate;}).sort(datedDesc).slice(0,3);
  ...
  for(const row of [...(targetTraining?[targetTraining]:[]),...recentTrainings])for(const block of arr(row.payload?.session?.blocks||row.payload?.blocos)){
